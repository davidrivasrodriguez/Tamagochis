...
export class RoomService {
  ...
  public addPlayer(player: Player): Room {
    ...
    ServerService.getInstance().addPlayerToRoom(player.socket, room.name);
    if (room.players.length === RoomConfig.maxRoomPlayers) room.occupied = true;
    return room;
  }

  public findRoomByPlayerId(playerId: string) {
    return this.rooms.find((room) =>
      room.players.some((player) => player.id.toString() === playerId)
    );
  }
  public findRoomByName(roomName: string): Room | undefined {
